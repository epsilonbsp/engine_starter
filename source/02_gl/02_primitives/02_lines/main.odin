...
MAIN_VS :: GLSL_VERSION + `
    ...
    void main() {
        gl_Position = u_projection * vec4(i_position + u_offset, 0.0, 1.0);
        v_color = i_color;
    }
`
...
main :: proc() {
    ...
    gl.BufferData(gl.ARRAY_BUFFER, point_count * size_of(Vertex), &points[0], gl.STATIC_DRAW)
    ...
    loop: for {
        ...
        gl.Uniform2f(main_uf["u_offset"].location, -256, 0)
        gl.DrawArrays(gl.LINES, 0, i32(point_count))

        gl.Uniform2f(main_uf["u_offset"].location, 0, 0)
        gl.DrawArrays(gl.LINE_STRIP, 0, i32(point_count))

        gl.Uniform2f(main_uf["u_offset"].location, 256, 0)
        gl.DrawArrays(gl.LINE_LOOP, 0, i32(point_count))
