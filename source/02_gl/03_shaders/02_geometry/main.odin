...
MAIN_GS :: GLSL_VERSION + `
    ...
    void main() {
        for (int i = 0; i < 4; i++) {
            gl_Position = u_projection * vec4(POSITIONS[i], 0.0, 1.0);
            v_tex_coord = TEX_COORDS[i];
            v_color = COLORS[i];
            EmitVertex();
        }
...
main :: proc() {
    ...
    main_pg, main_ok := load_shaders_source(MAIN_VS, MAIN_GS, MAIN_FS); defer gl.DeleteProgram(main_pg)
