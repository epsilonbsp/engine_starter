...
MAIN_VS :: GLSL_VERSION + `
    ...
    uniform mat3x2 u_model;

    const vec2 QUAD_SIZE = vec2(128.0);
    ...
    void main() {
        vec2 position = u_model * vec3(POSITIONS[gl_VertexID], 1.0);
...
make_transform :: proc(translation: glm.vec2, rotation: f32, scale: glm.vec2, shear: glm.vec2 = {}) -> glm.mat3x2 {
    c := glm.cos(rotation)
    s := glm.sin(rotation)

    rx := glm.vec2{ c, s} * scale.x
    ry := glm.vec2{-s, c} * scale.y
