...
WINDOW_TITLE :: "Sharpen"
WINDOW_WIDTH :: 960
WINDOW_HEIGHT :: 540
GL_VERSION_MAJOR :: 4
GL_VERSION_MINOR :: 6

SHARPEN_VS :: b.LIGHTING_VS

SHARPEN_FS :: b.GLSL_VERSION + `
    in vec2 v_tex_coord;
    ...
    void main() {
        ...
        if (u_enable_pp) {
            ...
            vec3 blur =
                texture(u_scene_color, v_tex_coord + vec2( texel.x, 0.0)).rgb +
                texture(u_scene_color, v_tex_coord + vec2(-texel.x, 0.0)).rgb +
                texture(u_scene_color, v_tex_coord + vec2(0.0, texel.y)).rgb +
                texture(u_scene_color, v_tex_coord + vec2(0.0, -texel.y)).rgb;

            blur *= 0.25;

            color = color + u_sharpen_strength * (color - blur);
        }
...
main :: proc() {
    ...
    loop: for {
        ...
        for sdl.PollEvent(&event) {
            ...
            case .KEY_DOWN:
                ...
                if event.key.scancode == sdl.Scancode.UP {
                    ...
                    fmt.printf("Sharpen strength: %.1f\n", sharpen_strength)
                }

                if event.key.scancode == sdl.Scancode.DOWN {
                    sharpen_strength = max(0.0, sharpen_strength - 0.5)
        ...
        gl.UseProgram(sharpen_pg)
        gl.ActiveTexture(gl.TEXTURE0)
        gl.BindTexture(gl.TEXTURE_2D, base.scene_buffer.color_tex)
        gl.Uniform1i(sharpen_uf["u_scene_color"].location, 0)
        gl.Uniform1f(sharpen_uf["u_exposure"].location, base.exposure)
        gl.Uniform1f(sharpen_uf["u_sharpen_strength"].location, sharpen_strength)
        gl.Uniform1i(sharpen_uf["u_debug_buffer"].location, base.debug_buffer)
        gl.Uniform1i(sharpen_uf["u_enable_pp"].location, i32(enable_pp))
