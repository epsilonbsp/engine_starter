...
WINDOW_TITLE :: "Exposure"
WINDOW_WIDTH :: 960
WINDOW_HEIGHT :: 540
GL_VERSION_MAJOR :: 4
GL_VERSION_MINOR :: 6

EXPOSURE_VS :: b.LIGHTING_VS

EXPOSURE_FS :: b.GLSL_VERSION + `
    in vec2 v_tex_coord;
...
main :: proc() {
    ...
    exposure_pg, exposure_ok := gl.load_shaders_source(EXPOSURE_VS, EXPOSURE_FS); defer gl.DeleteProgram(exposure_pg)
    exposure_uf := gl.get_uniforms_from_program(exposure_pg); defer gl.destroy_uniforms(exposure_uf);
    assert(exposure_ok, "ERROR: Failed to compile program")
    ...
    loop: for {
        ...
        gl.UseProgram(exposure_pg)
        gl.ActiveTexture(gl.TEXTURE0)
        gl.BindTexture(gl.TEXTURE_2D, base.scene_buffer.color_tex)
        gl.Uniform1i(exposure_uf["u_scene_color"].location, 0)
        gl.Uniform1f(exposure_uf["u_exposure"].location, base.exposure)
        gl.Uniform1i(exposure_uf["u_debug_buffer"].location, base.debug_buffer)
        gl.Uniform1i(exposure_uf["u_enable_pp"].location, i32(enable_pp))
