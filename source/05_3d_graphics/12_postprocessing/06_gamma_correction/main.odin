...
GAMMA_CORRECTION_VS :: b.LIGHTING_VS

GAMMA_CORRECTION_FS :: b.GLSL_VERSION + `
    in vec2 v_tex_coord;
...
main :: proc() {
    ...
    loop: for {
        ...
        gl.UseProgram(gamma_correction_pg)
        gl.ActiveTexture(gl.TEXTURE0)
        gl.BindTexture(gl.TEXTURE_2D, base.scene_buffer.color_tex)
        gl.Uniform1i(gamma_correction_uf["u_scene_color"].location, 0)
        gl.Uniform1f(gamma_correction_uf["u_exposure"].location, base.exposure)
        gl.Uniform1i(gamma_correction_uf["u_debug_buffer"].location, base.debug_buffer)
        gl.Uniform1i(gamma_correction_uf["u_enable_pp"].location, i32(enable_pp))
