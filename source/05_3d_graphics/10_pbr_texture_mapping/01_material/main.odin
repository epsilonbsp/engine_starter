...
MAIN_FS :: GLSL_VERSION + `
    ...
    void main() {
        ...
        float metallic = texture(u_metallic_tex, v_tex_coord).r * u_mat_metallic;
        float roughness = texture(u_roughness_tex, v_tex_coord).r * u_mat_roughness;
        float ao = texture(u_ao_tex, v_tex_coord).r * u_mat_ao;
...
main :: proc() {
    ...
    metallic_tex := load_texture_from_bytes(#load("textures/metallic.png")); defer gl.DeleteTextures(1, &metallic_tex)
    ...
    loop: for {
        ...
        gl.BindTexture(gl.TEXTURE_2D, metallic_tex)
        gl.Uniform1i(main_uf["u_metallic_tex"].location, 1)

        gl.ActiveTexture(gl.TEXTURE2)
        gl.BindTexture(gl.TEXTURE_2D, roughness_tex)
        gl.Uniform1i(main_uf["u_roughness_tex"].location, 2)

        gl.ActiveTexture(gl.TEXTURE3)
        gl.BindTexture(gl.TEXTURE_2D, ao_tex)
        gl.Uniform1i(main_uf["u_ao_tex"].location, 3)
