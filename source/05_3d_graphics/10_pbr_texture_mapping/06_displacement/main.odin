...
generate_cube :: proc(div: int) -> (vertices: []Vertex, indices: []u32) {
    fn := f32(div)
    verts_per_face := (div + 1) * (div + 1)

    vertices = make([]Vertex, 6 * verts_per_face)
    indices = make([]u32, 6 * div * div * 6)
    ...
    for f, fi in faces {
        ...
        for j in 0 ..= div {
            for i in 0 ..= div {
                ...
                vertices[vi] = Vertex{
                    position = f.origin + f.u_dir * u + f.v_dir * v,
                    normal = f.normal,
                    tex_coord = {u, v},
                    tangent = {f.u_dir.x, f.u_dir.y, f.u_dir.z, -1}
                }
        ...
        for j in 0 ..< div {
            for i in 0 ..< div {
                v00 := base + u32(j * (div + 1) + i)
                v10 := base + u32(j * (div + 1) + (i + 1))
                v01 := base + u32((j + 1) * (div + 1) + i)
                v11 := base + u32((j + 1) * (div + 1) + (i + 1))

                indices[ii + 0] = v01; indices[ii + 1] = v11; indices[ii + 2] = v10
                indices[ii + 3] = v01; indices[ii + 4] = v10; indices[ii + 5] = v00
...
MAIN_VS :: GLSL_VERSION + `
    ...
    void main() {
        ...
        vec3 displaced = i_position + i_normal * height * mask * u_height_scale;
...
main :: proc() {
    ...
    mesh_vertices, mesh_indices := generate_cube(64)
    ...
    // Source: https://polyhaven.com/a/rock_wall_10
