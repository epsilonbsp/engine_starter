...
WINDOW_TITLE :: "Displacement Tessellation"
WINDOW_WIDTH :: 960
WINDOW_HEIGHT :: 540
GL_VERSION_MAJOR :: 4
GL_VERSION_MINOR :: 6
...
MAIN_TES :: GLSL_VERSION + `
    ...
    void main() {
        ...
        vec3 i_normal = gl_TessCoord.x * tc_normal[0] + gl_TessCoord.y * tc_normal[1] + gl_TessCoord.z * tc_normal[2];
        vec2 i_tex_coord = gl_TessCoord.x * tc_tex_coord[0] + gl_TessCoord.y * tc_tex_coord[1] + gl_TessCoord.z * tc_tex_coord[2];
        vec4 i_tangent = gl_TessCoord.x * tc_tangent[0] + gl_TessCoord.y * tc_tangent[1] + gl_TessCoord.z * tc_tangent[2];

        i_normal = normalize(i_normal);

        float height = texture(u_displacement_tex, i_tex_coord).r;
        float border = 0.05;
        float mask = smoothstep(0.0, border, i_tex_coord.x) * smoothstep(1.0, 1.0 - border, i_tex_coord.x)
                   * smoothstep(0.0, border, i_tex_coord.y) * smoothstep(1.0, 1.0 - border, i_tex_coord.y);
        vec3 displaced = position + i_normal * height * mask * u_height_scale;
...
main :: proc() {
    ...
    // Source: https://polyhaven.com/a/cobblestone_floor_04
    ...
    loop: for {
        ...
        for sdl.PollEvent(&event) {
            ...
            case .KEY_DOWN:
                ...
                if event.key.scancode == sdl.Scancode._1 {
                    tess_level = max(tess_level - 1, 1)
                }

                if event.key.scancode == sdl.Scancode._2 {
                    tess_level = min(tess_level + 1, 64)
                }
            case .MOUSE_MOTION:
        ...
        for &mesh in meshes {
            ...
            gl.DrawElements(gl.PATCHES, i32(mesh_index_count), gl.UNSIGNED_INT, nil)
        }
