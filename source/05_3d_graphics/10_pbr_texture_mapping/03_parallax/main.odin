...
MAIN_FS :: GLSL_VERSION + `
    ...
    vec2 parallax_mapping(vec2 tc, vec3 tangent_v) {
        float height = 1.0 - texture(u_displacement_tex, tc).r;

        return tc - tangent_v.xy * (height * u_height_scale);
    }
...
main :: proc() {
    ...
    // Source: https://polyhaven.com/a/random_bricks_thick
