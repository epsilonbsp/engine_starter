...
WINDOW_TITLE :: "Parallax Occlusion"
WINDOW_WIDTH :: 960
WINDOW_HEIGHT :: 540
GL_VERSION_MAJOR :: 4
GL_VERSION_MINOR :: 6
...
MAIN_FS :: GLSL_VERSION + `
    ...
    vec2 parallax_mapping(vec2 tc, vec3 tangent_v) {
        ...
        float current_height = 1.0 - texture(u_displacement_tex, tc).r;

        while (current_layer_depth < current_height) {
            tc -= delta_tc;
            current_height = 1.0 - texture(u_displacement_tex, tc).r;
            current_layer_depth += layer_depth;
        }

        // Interpolate between last two layers to smooth the intersection
        vec2 prev_tc = tc + delta_tc;
        float after_depth = current_height - current_layer_depth;
        float before_depth = 1.0 - texture(u_displacement_tex, prev_tc).r - current_layer_depth + layer_depth;
        float weight = after_depth / (after_depth - before_depth);

        return prev_tc * weight + tc * (1.0 - weight);
    }
