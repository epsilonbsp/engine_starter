...
MAIN_FS :: GLSL_VERSION + `
    ...
    void main() {
        ...
        for (int i = 0; i < LIGHTS_CAP; i++) {
            vec3 light_dir = normalize(u_lights[i].position - v_world_pos);
            vec3 half_dir = normalize(light_dir + view_dir);

            float distance = length(u_lights[i].position - v_world_pos);
            float attenuation = 1.0 / (u_lights[i].constant + u_lights[i].linear * distance + u_lights[i].quadratic * distance * distance);

            vec3 ambient = u_mat_color * u_lights[i].color * u_mat_ambient_strength;
            vec3 diffuse = u_mat_color * u_lights[i].color * max(dot(normal, light_dir), 0.0) * u_mat_diffuse_strength;
            vec3 specular = u_lights[i].color * pow(max(dot(normal, half_dir), 0.0), u_mat_specular_shine) * u_mat_specular_strength;
...
main :: proc() {
    ...
    loop: for {
        ...
        for i in 0 ..< LIGHTS_CAP {
            pos_key := fmt.tprintf("u_lights[%d].position", i)
            color_key := fmt.tprintf("u_lights[%d].color", i)
            const_key := fmt.tprintf("u_lights[%d].constant", i)
            linear_key := fmt.tprintf("u_lights[%d].linear", i)
            quad_key := fmt.tprintf("u_lights[%d].quadratic", i)
            gl.Uniform3fv(main_uf[pos_key].location,   1, &lights[i].position[0])
            gl.Uniform3fv(main_uf[color_key].location, 1, &lights[i].color[0])
            gl.Uniform1f(main_uf[const_key].location,  lights[i].constant)
            gl.Uniform1f(main_uf[linear_key].location, lights[i].linear)
            gl.Uniform1f(main_uf[quad_key].location,   lights[i].quadratic)
        }
