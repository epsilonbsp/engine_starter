...
meshes := []Mesh {
    {{-20, 0, 0}, {0, 0, 0}, {2, 2, 2}, {{0.8, 0.5, 0.3}, 0.02, 1.0, 0.0, 1.0  }},
    {{-12, 0, 0}, {0, 0, 0}, {2, 2, 2}, {{0.2, 0.4, 0.8}, 0.02, 0.9, 0.5, 32.0 }},
    {{-4,  0, 0}, {0, 0, 0}, {2, 2, 2}, {{0.7, 0.7, 0.7}, 0.02, 0.6, 1.0, 256.0}},
    {{ 4,  0, 0}, {0, 0, 0}, {2, 2, 2}, {{0.5, 0.8, 0.3}, 0.02, 1.0, 0.3, 16.0 }},
    {{ 12, 0, 0}, {0, 0, 0}, {2, 2, 2}, {{0.8, 0.3, 0.5}, 0.02, 0.9, 0.6, 64.0 }},
    {{ 20, 0, 0}, {0, 0, 0}, {2, 2, 2}, {{0.6, 0.6, 0.4}, 0.02, 1.0, 0.0, 1.0  }},
}
...
MAIN_FS :: GLSL_VERSION + `
    ...
    void main() {
        ...
        vec3 result = mix(u_fog_color, light, fog_factor);
...
main :: proc() {
    ...
    loop: for {
        ...
        fog_color := glm.pow(fog.color, 1.0 / 2.0)

        gl.Viewport(0, 0, viewport_x, viewport_y)
        gl.ClearColor(fog_color.r, fog_color.g, fog_color.b, 1.0)
        ...
        gl.Uniform3fv(main_uf["u_fog_color"].location, 1, &fog.color[0])
        gl.Uniform1f(main_uf["u_fog_start"].location, fog.start)
        gl.Uniform1f(main_uf["u_fog_end"].location, fog.end)
