...
MAIN_FS :: GLSL_VERSION + `
    ...
    void main() {
        ...
        vec3 specular_strength = u_mat_specular_strength * texture(u_specular_tex, v_tex_coord).rgb;
        ...
        vec3 specular = u_light_color * pow(max(dot(normal, half_dir), 0.0), u_mat_specular_shine) * specular_strength;
...
main :: proc() {
    ...
    specular_tex := load_texture_from_bytes(#load("textures/specular.png")); defer gl.DeleteTextures(1, &specular_tex)
    ...
    loop: for {
        ...
        gl.Uniform1i(main_uf["u_diffuse_tex"].location, 0)

        gl.ActiveTexture(gl.TEXTURE1)
        gl.BindTexture(gl.TEXTURE_2D, specular_tex)
        gl.Uniform1i(main_uf["u_specular_tex"].location, 1)
