...
MAIN_FS :: GLSL_VERSION + `
    ...
    void main() {
        ...
        float opacity = texture(u_opacity_tex, v_tex_coord).r;
        ...
        o_frag_color = vec4(pow(result, vec3(1.0 / 2.2)), opacity);
    }
`
...
main :: proc() {
    ...
    opacity_tex := load_texture_from_bytes(#load("textures/opacity.png")); defer gl.DeleteTextures(1, &opacity_tex)
    ...
    loop: for {
        ...
        gl.BindTexture(gl.TEXTURE_2D, opacity_tex)
        gl.Uniform1i(main_uf["u_opacity_tex"].location, 1)
