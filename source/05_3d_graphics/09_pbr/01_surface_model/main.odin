...
MAIN_FS :: GLSL_VERSION + `
    ...
    void main() {
        ...
        vec3 f0 = mix(vec3(0.04), u_mat_albedo, u_mat_metallic);

        // Cook-torrance BRDF terms
        float ndf = distribution_ggx(n, h, u_mat_roughness);
        float g = geometry_smith(n, v, l, u_mat_roughness);
        vec3 f = fresnel_schlick(clamp(dot(h, v), 0.0, 1.0), f0);

        // Diffuse, metals have no diffuse
        vec3 kd = (vec3(1.0) - f) * (1.0 - u_mat_metallic);
        ...
        vec3 lo = (kd * u_mat_albedo / PI + specular) * u_light_color * n_dot_l;

        // Ambient
        vec3 ambient = vec3(0.03) * u_mat_albedo * u_mat_ao;
