...
OUTLINE_VS :: GLSL_VERSION + `
    ...
    void main() {
        vec3 expanded = i_position + i_normal * THICKNESS;
        gl_Position = u_projection * u_view * u_model * vec4(expanded, 1.0);
    }
`

OUTLINE_FS :: GLSL_VERSION + `
    out vec4 o_frag_color;

    const vec3 OUTLINE_COLOR = vec3(1.0, 0.85, 0.0);

    void main() {
        o_frag_color = vec4(OUTLINE_COLOR, 1.0);
    }
`
...
main :: proc() {
    ...
    outline_pg, outline_ok := gl.load_shaders_source(OUTLINE_VS, OUTLINE_FS); defer gl.DeleteProgram(outline_pg)
    outline_uf := gl.get_uniforms_from_program(outline_pg); defer gl.destroy_uniforms(outline_uf)
    assert(outline_ok, "ERROR: Failed to compile program")
    ...
    loop: for {
        ...
        // Draw meshes, writing 1 to stencil
        ...
        // Draw outlines where stencil != 1
        gl.StencilFunc(gl.NOTEQUAL, 1, 0xFF)
        gl.StencilMask(0x00)
        gl.Disable(gl.DEPTH_TEST)

        gl.UseProgram(outline_pg)
        gl.UniformMatrix4fv(outline_uf["u_projection"].location, 1, false, &camera.projection[0][0])
        gl.UniformMatrix4fv(outline_uf["u_view"].location, 1, false, &camera.view[0][0])

        for &mesh in meshes {
            model := make_transform(mesh.translation, mesh.rotation, mesh.scale)
            gl.UniformMatrix4fv(outline_uf["u_model"].location, 1, false, &model[0][0])
            gl.DrawElements(gl.TRIANGLES, i32(mesh_index_count), gl.UNSIGNED_INT, nil)
        }
